...
// Helper for LRU eviction (Part C)
...
self.addEventListener('fetch', e => {
  ...
  // FIX 2 - Part A & B: Images (network-first, credentials omit, 24h cache)
    ...
    e.respondWith(
      fetch(new Request(e.request.url, { credentials: 'omit', mode: 'cors' }))
        ...
        .catch(async () => {
          ...
          // Part A - Fallback: transparent 1x1 pixel GIF
          return new Response(
  ...
  // API calls (network-first, 1h cache)
  if (isWP && url.pathname.includes('/wp-json/')) {
    // If it's a lightbox detail call (per requirements in Change 1/3) -> network-only
